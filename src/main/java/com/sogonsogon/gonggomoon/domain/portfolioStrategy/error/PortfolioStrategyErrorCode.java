package com.sogonsogon.gonggomoon.domain.portfolioStrategy.error;
...
public enum PortfolioStrategyErrorCode implements BaseErrorCode {
    ...
    POST_ID_REQUIRED("PORTFOLIO_STRATEGY_POST_ID_REQUIRED", HttpStatus.BAD_REQUEST, "공고 아이디는 필수입니다."),
    ...
    WEEKLY_LIMIT_EXCEEDED("PORTFOLIO_STRATEGY_WEEKLY_LIMIT_EXCEEDED", HttpStatus.CONFLICT, "이번 주 포트폴리오 전략 생성 가능 횟수를 모두 사용했습니다."),
    INVALID_STATUS("PORTFOLIO_STRATEGY_INVALID_STATUS", HttpStatus.CONFLICT, "현재 상태에서는 포트폴리오 전략 생성을 시작할 수 없습니다."),
    RESULT_JSON_EMPTY("PORTFOLIO_STRATEGY_RESULT_JSON_EMPTY", HttpStatus.INTERNAL_SERVER_ERROR, "전략 결과 JSON이 비어 있습니다."),
    RESULT_NOT_READY("PORTFOLIO_STRATEGY_RESULT_NOT_READY", HttpStatus.ACCEPTED, "전략 결과가 아직 준비되지 않았습니다."),
    GENERATION_FAILED("PORTFOLIO_STRATEGY_GENERATION_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "포트폴리오 전략 생성에 실패했습니다.")
    ;
