...
public class PortfolioStrategyController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400",
                    description = "입력값 검증 실패(GLOBAL_INVALID_INPUT_VALUE) / 올바르지 않은 직무 타입(PORTFOLIO_STRATEGY_INVALID_JOB_TYPE) / 경험 선택 필수(PORTFOLIO_STRATEGY_EXPERIENCE_IDS_REQUIRED) / 요청한 경험을 찾을 수 없음(PORTFOLIO_STRATEGY_REQUESTED_EXPERIENCE_NOT_FOUND)",
            ...
            @ApiResponse(responseCode = "404",
                    description = "존재하지 않는 산업(INDUSTRY_NOT_FOUND)",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = BaseResponse.class),
                            examples = @ExampleObject(value = ErrorResponseExamples.INDUSTRY_NOT_FOUND))),
